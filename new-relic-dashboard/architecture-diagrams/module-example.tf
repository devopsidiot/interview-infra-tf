module "devopsidiot_new_relic_one_dashboard" {
  source = "git@github.com:devopsidiot/interview-infra-tf.git//new-relic-one-dashboard"
  ## Provider Values: ##
  region = "us-east-1"
  ## Dashboard Widget Values: ##
  newrelic_dashboard_name        = "devopsidiot-tf-test"
  ...
  newrelic_dashboard_pages = {
    "Infrastructure - Node Data" = {
      line_widgets = {
        ...
        "Node MemoryPressure" = {
          widget_line_nrql   = "SELECT latest(kube_node_status_condition) FROM Metric WHERE condition = 'MemoryPressure' TIMESERIES FACET `node` LIMIT 10 SINCE 1800 seconds ago"
          widget_line_column = 5
          widget_line_row    = 4
          widget_line_height = 3
          widget_line_width  = 4
        ...
        "Node PIDPressure" = {
          widget_line_nrql   = "SELECT latest(kube_node_status_condition) FROM Metric WHERE condition = 'PIDPressure' TIMESERIES FACET `node` LIMIT 10 SINCE 1800 seconds ago"
          widget_line_column = 1
          widget_line_row    = 7
          widget_line_height = 3
          widget_line_width  = 4
        ...
        "Node DiskPressure" = {
          widget_line_nrql   = "SELECT latest(kube_node_status_condition) FROM Metric WHERE condition = 'DiskPressure' TIMESERIES FACET `node` LIMIT 10 SINCE 1800 seconds ago"
          widget_line_column = 1
          widget_line_row    = 4
          widget_line_height = 3
          widget_line_width  = 4
      ...
      bar_widgets = {
        "Node Ready" = {
          widget_bar_nrql   = "SELECT latest(kube_node_status_condition) FROM Metric WHERE condition = 'Ready' FACET `node` LIMIT 10 SINCE 1800 seconds ago"
          widget_bar_column = 9
          widget_bar_row    = 1
          widget_bar_height = 3
          widget_bar_width  = 2
      ...
      pie_widgets = {
        "Unscheduable Nodes" = {
          widget_pie_nrql   = "SELECT latest(kube_node_spec_unschedulable) FROM Metric FACET `node` LIMIT 10 SINCE 1800 seconds ago"
